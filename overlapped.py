...
def load_data(xsize,ysize):
    ...
    new_train_x = numpy.zeros((train_labels.shape[0], (28-xsize+1) * (28-ysize+1)))
    for i in xrange((28-xsize+1) * (28-ysize+1)):
        if i % 28 != 0:
            cnt=0
            for row in xrange(xsize):
                for col in xrange(ysize):
                    new_train_x[:, i] += pow(2,cnt)*train_x[:, i+28*row+col]
                    cnt+=1
    ...
    new_test_x = numpy.zeros((test_labels.shape[0], (28-xsize+1) * (28-ysize+1)))
    for i in xrange((28-xsize+1) * (28-ysize+1)):
        if i % 28 != 0:
            cnt=0
            for row in xrange(xsize):
                for col in xrange(ysize):
                    new_test_x[:, i] += pow(2,cnt)*test_x[:, i+28*row+col]
                    cnt+=1
...
class BayesClassifier(object):
    ...
    def fit(self, X, y):
        ...
        bayesmatrix = numpy.ones((10, self.valuerange, (28-self.xsize+1) * (28-self.ysize+1)), dtype=numpy.float64)
        for k in xrange(10):
